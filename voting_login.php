...
if(isset($_POST['submit'])) {
    ...
    if(($username==$user)&&($password==$pass)) {
        $voter_id = $row['v_id'];
        $sql_2 = mysqli_query($con, "SELECT * FROM `admin` WHERE `v_id`='$voter_id'");
        $row_2 = mysqli_fetch_array($sql_2);
        if ($row_2['v_d']) {
            echo "<script>alert('Vote Already Completed')</script>";
        } else {
            echo "<script>window.location.href='voting_screen1.php'</script>";
        }
    }
    else {
...
<body>
    ...
    <h2 class="heading">Voting Login/Signup</h2>
    <form method="POST">
